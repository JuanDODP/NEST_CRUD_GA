...
export class UserRolGuard implements CanActivate {
  ...
  canActivate(
    ...
    // Obtenemos los roles permitidos para la ruta desde el decorador
    const validRoles: string[] = this.reflector.get( META_ROLES , context.getHandler() );

    // Si no se definieron roles requeridos, permitimos el acceso
    ...
    // POSTGRESQL: user.rol ya es un string[], simplemente iteramos
    for ( const role of user.rol ) {
      if ( validRoles.includes( role ) ) {
        return true;
      }
